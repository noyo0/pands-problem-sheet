...
# Program finds square root of a given number with Newton-Raphson Method
...
# Program runs until max accuracy is reached;
    # I noticed that while infinite iterations, python limits the number of digits at 17 and after reaching max accuracy within this 17 digit restraijnt, the results are repeating.
    # Exploiting this "feature" the program can store results in a container and check current result against previous iteration. 
    # If the two are the same, there is no need to iterate any more, the result is the most accurate and final the program can break out of the loop.
...
if f== "Y" or f=="y" or f=="yes" or f=="Yes": # Program allows for few types of "yes" entries for Floating point display, everything else results in integer display
    ...
    f=1
p=fn_newton(n,f) # setting variable p for printing the result
